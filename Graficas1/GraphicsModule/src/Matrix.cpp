...
void Matrix::MatrixMultiplication(const float* _MatrixA, const float* _MatrixB, float* _Result)
{
    ...
    //Product[5] = (_MatrixA[4] * _MatrixB[1]) + (_MatrixA[5] * _MatrixB[5]) + (_MatrixA[6] * _MatrixB[9]) + (_MatrixA[7] * _MatrixB[13]);
    //Product[6] = (_MatrixA[4] * _MatrixB[2]) + (_MatrixA[5] * _MatrixB[6]) + (_MatrixA[6] * _MatrixB[10]) + (_MatrixA[7] * _MatrixB[14]);
    //Product[7] = (_MatrixA[4] * _MatrixB[3]) + (_MatrixA[5] * _MatrixB[7]) + (_MatrixA[6] * _MatrixB[11]) + (_MatrixA[7] * _MatrixB[15]);
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
